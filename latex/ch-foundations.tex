...
\begin{proposition}[Composition of Injection]
    Given injections $f \colon X \mapsto Y$ and $g \colon Y \mapsto Z$, then their \textit{composition} $h \colon X \mapsto Z$ is given by
    \begin{equation}
        h(x) = g(f(x))
    \end{equation}
    Namely $h = g \circ f$ where $h$ is a function composed from $g$ and $f$ with $f$ applied first.
\end{proposition}
